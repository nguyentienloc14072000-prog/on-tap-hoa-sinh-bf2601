(() => {
  ...
  const courseStores={};
  let course,modules,questions,questionMap,key,store;
  ...
  function activateCourse(id) {
    const next=courseMap[id]||courses[0];
    if(course===next)return;
    course=next;
    modules=allModules.filter(m=>m.courseId===course.id);
    questions=modules.flatMap(m=>m.questions.map(q=>({...q,moduleId:m.id})));
    questionMap=Object.fromEntries(questions.map(q=>[q.id,q]));
    key=course.id+'-review-v1';
    if(!courseStores[course.id]) {
      let saved=null;
      try{saved=JSON.parse(localStorage.getItem(key)||'null');}catch{/* Use a fresh store if unavailable. */}
      courseStores[course.id]=StudyState.restore(saved,course,questionMap);
    }
    store=courseStores[course.id];reviewOnlyWrong=false;
    document.getElementById('course-switcher').innerHTML=courses.map(c=>`<a href="#course/${c.id}" class="course-tab" ${c.id===course.id?'aria-current="page"':''}><span>${esc(c.code)}</span><strong>${esc(c.title)}</strong></a>`).join('');
    document.querySelector('.brand').href='#course/'+course.id;
    document.querySelector('.topnote').href='#course/'+course.id;
    document.getElementById('source-note').textContent=course.code+' · '+course.sourceFolder+' · Trang nguồn tính theo trang PDF.';
    document.title=course.title+' · Ôn tập '+course.code;
  }
  function groups(){return [...new Set(modules.map(m=>m.group))];}
  function groupScope(group){return 'group:'+group;}
  function scopes(){return ['all',...groups().map(groupScope),...(course.id==='bf2601'?['enzym']:[]),...modules.map(m=>m.id)];}
  ...
  function titleForScope(scope) {
    return scope==='all'?course.title:scope==='enzym'?'Enzym · cả 4 phần':scope.startsWith('group:')?scope.slice(6):moduleMap[scope]?.title||'Ôn lại câu sai';
  }
  function poolFor(scope) {
    return scope==='all'?questions:scope==='enzym'?questions.filter(q=>moduleMap[q.moduleId].group==='Enzym học lâm sàng'):scope.startsWith('group:')?questions.filter(q=>moduleMap[q.moduleId].group===scope.slice(6)):questions.filter(q=>q.moduleId===scope);
  }
  ...
  function home() {
    ...
    app.innerHTML=`<div class="heading"><div><p class="eyebrow">${esc(course.code)} · ${esc(course.sourceFolder)}</p><h1>${esc(course.title)}</h1><p class="subtle">Đọc lại kiến thức, làm trắc nghiệm và đối chiếu với slide gốc.</p></div><button class="button primary" data-action="setup" data-scope="all">Ôn tổng hợp <span aria-hidden="true">→</span></button></div>
  ...
  function quiz() {
    const s=store.session;if(!s){moveTo('#course/'+course.id);return;}if(s.status==='complete'){results();return;}
  ...
  document.addEventListener('click',e=>{
    ...
    switch(b.dataset.action){
      ...
      case 'resume':moveTo('#quiz/'+course.id);break;
  ...
  function render(){
    ...
    setup.close();finishDialog.close();
    if(route.startsWith('#study/')){
      const id=route.slice(7);activateCourse(moduleMap[id]?.courseId);study(id);
    }else if(route==='#quiz'||route.startsWith('#quiz/')){
      activateCourse(route==='#quiz'?'bf2601':route.slice(6));quiz();
    }else{
      activateCourse(route.startsWith('#course/')?route.slice(8):(course?.id||'bf2601'));home();
    }
    app.removeAttribute('aria-busy');
  }
  ...
  if(context?.registerTool){
    ...
    const tools=[
      {name:'list_study_materials',title:'Liệt kê tài liệu ôn tập',description:'List supplied lecture materials grouped by course.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true,untrustedContentHint:false},execute:()=>courses.map(c=>({id:c.id,title:c.title,materials:allModules.filter(m=>m.courseId===c.id).map(m=>({id:m.id,title:m.title,pages:m.source.pages,questions:m.questions.length}))}))},
      {name:'open_quiz_setup',title:'Mở thiết lập bài ôn',description:'Open the visible quiz setup for the requested course and scope. Does not start or submit a quiz.',inputSchema:{type:'object',properties:{courseId:{type:'string',enum:courses.map(c=>c.id)},scope:{type:'string',enum:['all','enzym',...new Set(allModules.map(m=>groupScope(m.group))),...allModules.map(m=>m.id)]}},required:['courseId','scope'],additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:false},execute:input=>{
        if(!input||!courseMap[input.courseId]||typeof input.scope!=='string')throw new Error('Môn học hoặc phạm vi không hợp lệ.');
        const requested=allModules.filter(m=>m.courseId===input.courseId);
        const valid=['all',...(input.courseId==='bf2601'?['enzym']:[]),...requested.map(m=>m.id),...requested.map(m=>groupScope(m.group))];
        if(!valid.includes(input.scope))throw new Error('Phạm vi không thuộc môn học đã chọn.');
        if(finishDialog.open)throw new Error('Hãy hoàn tất hộp thoại nộp bài trước.');
        if(setup.open)setup.close();
        history.replaceState(null,'','#course/'+input.courseId);render();openSetup(input.scope);
        return{status:'setup_open',courseId:course.id,scope:input.scope,availableQuestions:poolFor(input.scope).length};
      }}
    ];
